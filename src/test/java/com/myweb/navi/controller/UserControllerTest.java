...
public class UserControllerTest {
	...
	void 이메일중복체크_거짓() throws Exception {
		this.mockmvc
				.perform(get("/users/exist").param("email", "abc@naver.com")
				.contentType(MediaType.APPLICATION_JSON))
	...
	void 이메일중복체크_참() throws Exception {
		this.mockmvc
				.perform(get("/users/exist").param("email", "bd@naver.com")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.unique", "false").exists())
				.andDo(print());
	...
	void 잘못된_이메일값체크() throws Exception {
		this.mockmvc
				.perform(get("/users/exist").param("email", "bnaver.com")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isBadRequest())
				.andDo(print());
	}
	...
	void 닉네임중복체크_거짓() throws Exception {
		this.mockmvc
				.perform(get("/users/exist").param("nickname", "cdc")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.unique", "false").exists())
				.andDo(print());
	...
	void 닉네임중복체크_참() throws Exception {
		this.mockmvc
				.perform(get("/users/exist").param("nickname", "bcde")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.unique", "false").exists())
				.andDo(print());
	...
	void 잘못된_닉네임값체크() throws Exception {
		this.mockmvc
				.perform(get("/users/exist").param("nickname", "er.com")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isBadRequest())
				.andDo(print());
	}
	...
	void 회원정보조회() throws Exception {
		...
		this.mockmvc
				.perform(post("/users/info").contentType(MediaType.APPLICATION_JSON)
				.content(request)
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andDo(print());
	...
	void 비밀번호수정() throws Exception {
		PasswordRequest passwordRequest = PasswordRequest.builder()
				.email("abc@naver.com")
				.oldPassword("ab456789")
				.newPassword("ab456798")
				.build();
	...
	void 닉네임수정() throws Exception {
		...
		this.mockmvc
				.perform(patch("/users/nickname").contentType(MediaType.APPLICATION_JSON)
	...
	void 유저정보삭제() throws Exception {
		...
		this.mockmvc
				.perform(delete("/users").contentType(MediaType.APPLICATION_JSON)
