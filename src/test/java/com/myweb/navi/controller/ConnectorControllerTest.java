...
public class ConnectorControllerTest {
	...
	void 시간표검색() throws Exception {
		StationInfoRequest request = StationInfoRequest.builder()
				.stationId("MTRARA1A01")
				.upDownType("D")
				.build();
	...
	void 버스위치검색() throws Exception {
		...
		this.mockmvc
		.perform(post("/api/bus/lineId").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request)).contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk()).andDo(print());
	}
	...
	void GTP메세지() throws Exception {
		...
		this.mockmvc
		.perform(get("/api/gpt").contentType(MediaType.APPLICATION_JSON)
