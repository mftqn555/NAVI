...
public interface CommentMapper {
	...
	@Select("SELECT cno, bno, user_id, nickname, content, DATE_FORMAT(create_date, '%y.%m.%d. %H:%i') as create_date, "
			+ "IF(re_cno IS NULL, cno, re_cno) AS re_cno " + "FROM comment " + "WHERE bno = #{bno} "
			+ "ORDER BY re_cno asc " + "LIMIT #{offset}, #{postsPerPage}")
	...
	// 댓글 삭제
	@Delete("DELETE FROM comment WHERE cno=#{cno} OR re_cno=#{cno}")
